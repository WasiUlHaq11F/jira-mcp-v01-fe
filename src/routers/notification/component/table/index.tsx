...
import { notificationTableConfigDefault } from '../../data/notificationTableConfigDefault';
import { getNotifications } from '../../service';
import { notificationGetColumns } from './columns';

const NotificationCreate = lazy(() => import('../header-actions/create'));
const NotificationUpdate = lazy(() => import('../row-actions/update'));
const NotificationDelete = lazy(() => import('../row-actions/delete'));
const NotificationView = lazy(() => import('../row-actions/view'));

export const NotificationTable: React.FC = memo(() => {

  const {
    tableConfiguration,
    columnVisibility,
    tableHandlers
  } = useDataTableManager(NOTIFICATION_CONSTANTS.TABLE_CONFIG_KEY, notificationTableConfigDefault);

  const multiSortEnabled = tableConfiguration?.multiSort ?? false;

  // 1. Generate columns with multiSortEnabled
  const columns = useMemo(() => notificationGetColumns(multiSortEnabled), [multiSortEnabled]);
  ...
  const {
    ...
    queryKey: [NOTIFICATION_CONSTANTS.QUERY_KEY],
  ...
  return (
    <>
      <DataTable
        ...
        pageSizeOptions={[]}
        entityName={NOTIFICATION_CONSTANTS.ENTITY_NAME}
        renderToolbar={() => <>
          {/* No Search form only Column Config */}
          <DataTableSettingsColumns
            variant="toolbar"
            columnVisibility={columnVisibility}
            defaultTableConfig={notificationTableConfigDefault}
